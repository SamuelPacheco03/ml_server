...
export class ChurnKnnService {
  ...
  private async initializeModel(): Promise<void> {
    try {
      ...
      if (fs.existsSync(absolutePath)) {
        ...
        console.warn(`⚠️ Modelo ONNX KNN no encontrado en ${absolutePath}. Usando simulación.`);
        this.useOnnx = false;
      }
    } catch (error) {
  ...
  private predictSimulated(request: ChurnRequest): ChurnResponse {
    ...
    // aquí usamos la misma lógica: probabilidadClase = confianza de la predicción
